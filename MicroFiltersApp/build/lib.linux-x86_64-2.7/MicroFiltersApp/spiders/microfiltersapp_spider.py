...
class MicroFiltersAppSpider(BaseSpider):
    ...
    def __init__(self, *args, **kwargs):
        ...
        global imagesToKeep
        imagesToKeep = list(kwargs.get('SIZES'))
        global filterEnglishGlobal
    ...
    def csvFunc(self):
        ...
        for x in files:
            if os.path.isfile(str(Id) + '/' +x):
                with open(str(Id) + '/' + x, 'r') as f:
                    content = f.read()
                ...
                with open(str(Id) + '/1' + x, 'w') as f:
                    f.write(content)
                os.renames(str(Id) + '/1' + x, str(Id) + '/' + x.split('.')[0] + '.csv')
                f = open(str(Id) + '/' + x.split('.')[0] + '.csv', 'r')
                reader = csv.DictReader(f, fieldnames = ( "username","tweet","time","location", "latitute", "longitude", "image") )
                out = out = "[\n\t" + ",\n\t".join([json.dumps(row) for row in reader]) + "\n]"
                with open(str(Id) + '/1' + x + '.json', 'w') as f:
                    f.write(out)
                os.renames(str(Id) + '/1' + x + '.json', str(Id) + '/' + x.split('.')[0] + '.json')
    ...
    def parse(self, response):
        ...
        filePath = str(Id) + '/' + self.mediaType(linkName=linkName) + '.txt'
        #items = []
    ...
    def sortChronologically(self):
        ...
        for y in files:
            linesOfLines = []
            if os.path.isfile(str(Id)+ '/' +y):
                with open(str(Id)+ '/' + y, 'r') as f:
                    contentLines = f.readlines()
                    for line in contentLines:
                        lineToAdd = line.split('\t')
                        try:
                            intOne = int(lineToAdd[2].split(' ')[2])*86400
                            intTwo = int(lineToAdd[2].split(' ')[3].split(':')[0])*3600
                            intThree = int(lineToAdd[2].split(' ')[3].split(':')[1])*60
                            intFour = int(lineToAdd[2].split(' ')[3].split(':')[2])
                            lineToAdd.append(str(intOne + intTwo + intThree + intFour))
                        except IndexError:
                            lineToAdd.append("-1")
                        linesOfLines.append(lineToAdd)
                    linesOfLines.sort(key=lambda x: x[-1])
                with open(str(Id)+ '/' + y, 'w') as f:
                    strsToWrite = []
